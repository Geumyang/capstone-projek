...
with tab2:
    ...
    st.markdown("""
    Bagian ini menyajikan analisis komparatif kandungan gizi makanan Indonesia
    berdasarkan database yang digunakan sistem NutriCitra.
    Semua nilai nutrisi disajikan **per porsi standar** masing-masing makanan.
    """)
    st.divider()
    ...
    st.markdown(
        f"Bagaimana distribusi {len(df_foods)} makanan dalam database "
        "berdasarkan kategori tingkat kalorinya per porsi standar?"
    )
    ...
    st.info(
        f"Kategori terbanyak dalam database adalah **{top_kat['Kategori']}** "
        f"({top_kat['Jumlah']} makanan atau {round(top_kat['Jumlah']/len(df_foods)*100,1)}% dari total). "
        "Makanan berkategori rendah kalori umumnya adalah sayuran, buah, atau makanan berbasis kuah. "
        "Makanan tinggi kalori biasanya melibatkan proses penggorengan atau mengandung karbohidrat tinggi."
    )
    st.divider()
    ...
    st.markdown(
        "Makanan mana yang memiliki kandungan tertinggi dan terendah per porsi standar?"
    )
    ...
    if df_top.empty:
        ...
        st.markdown("**Temuan**")
        st.info(
            f"Makanan dengan kandungan {label.lower()} per porsi tertinggi dalam filter ini adalah "
            f"**{top1['food_name']}** ({top1[rank_by]:.1f} {unit}, {top1['serving_description']}). "
            f"Sementara yang terendah adalah **{last1['food_name']}** "
    ...
    st.markdown(
        "Makanan mana yang memberikan protein tertinggi dengan kalori yang relatif rendah? "
        "Indeks dihitung sebagai gram protein per 100 kkal dari satu porsi standar."
    )
    ...
    st.markdown("**Temuan**")
    st.info(
        f"**{top_eff['food_name']}** memiliki indeks efisiensi protein tertinggi "
        f"({top_eff['efisiensi_protein']:.2f} g protein per 100 kkal, {top_eff['serving_description']}), "
        f"mencerminkan kandungan protein yang tinggi dengan kalori yang relatif rendah. "
        f"Sebaliknya, **{last_eff['food_name']}** menunjukkan indeks terendah "
        f"({last_eff['efisiensi_protein']:.2f} g/100 kkal)."
    )
    st.divider()
...
with tab3:
    ...
    **Pertanyaan Bisnis:**
    ...
    st.success(
        ...
        "nilai precision dan recall di atas 0.95."
    )
    st.divider()
    ...
    st.markdown("**Temuan**")
    st.info(
        f"Kelas dengan mAP50 tertinggi: **{best['Kelas']}** ({best['mAP50']:.3f}). "
        f"Terendah: **{worst['Kelas']}** ({worst['mAP50']:.3f}). "
        f"Perbedaan ini mencerminkan variasi kesulitan deteksi akibat karakteristik visual tiap kelas."
    )
    st.markdown("**Implikasi**")
    st.info(
        f"Kelas **{worst['Kelas']}** menjadi prioritas peningkatan pada iterasi berikutnya, "
        f"baik melalui penambahan data, augmentasi lebih variatif, maupun penyesuaian arsitektur."
    )
    st.divider()
    ...
    st.markdown("**Temuan**")
    st.info(
        f"Korelasi antara jumlah gambar dan mAP50 adalah **{corr:.3f}** (hubungan {kuat}). "
        f"Kelas {worst['Kelas']} punya gambar terbanyak ({int(worst['Images'])}) tapi mAP50 terendah — "
        f"menunjukkan volume data bukan satu-satunya penentu akurasi."
    )
    st.divider()
    ...
    st.markdown("**Implikasi**")
    st.info(
        "Kelas di luar zona ideal memerlukan perhatian lebih dari sisi kualitas data dan augmentasi. "
        "Peningkatan recall diprioritaskan bila aplikasi menuntut sensitivitas deteksi tinggi."
    )
